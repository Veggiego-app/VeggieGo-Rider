...
export function readStopNode(xmlDoc, tagName, i){
    ...
    for (; i < xmlorder.length; i++) {
      if( xmlDoc[i] === "<"){
        if (xmlDoc[i+1] === "/") {//close tag
            const closeIndex = findSubStrIndex(xmlDoc, ">", i, `${tagName} is not closed`);
            let closeTagName = xmlorder.substring(i+2,closeIndex).trim();
            if(closeTagName === tagName){
              openTagCount--;
              if (openTagCount === 0) {
                return {
                  tagContent: xmlorder.substring(startIndex, i),
                  i : closeIndex
                }
              }
            }
